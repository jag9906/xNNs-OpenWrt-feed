...
# get_collected_info()
# Returns the most recently collected system information
...
my $updates = qx{wget -qO- --user-agent="Webmin" -t 1 --timeout=3 $webmin::update_host."/updates/" 2> /dev/null};
...
# Returns an array of times and values for some stat
...
# Creates or updates the Webmin function cron job
sub setup_collectinfo_job
{
&foreign_require("webmincron");
&foreign_require("webmin","webmin-lib.pl");
my $updates = qx{wget -qO- --user-agent="Webmin" -t 1 --timeout=3 $webmin::update_host 2> /dev/null};
